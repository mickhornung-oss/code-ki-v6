...
function getHtml(webview) {
  ...
  <body>
    ...
    <div id="agentAutonomyRow" class="agent-autonomy-row" hidden>
    ...
    <div class="card" id="v4Card" hidden>
      <strong>V4 Ablauf</strong>
      <pre id="v4Status"></pre>
      <div id="v4Plan"></div>
      <div id="v4Checkpoints"></div>
    </div>
    <div class="card" id="v6Card" hidden>
    ...
    <div class="card" id="v5Card" hidden>
      ...
      <div id="v5DryRunCard" class="change-item" hidden>
      ...
      <div id="v5DiffCard" class="change-item" hidden>
      ...
      <div id="v5RefineCard" class="change-item" hidden>
    ...
    <div class="card" id="explanationCard" hidden>
    ...
    <div class="card" id="changesCard" hidden>
      ...
      <div id="changes"></div>
      <button id="applyChanges" class="mt-10" disabled>Aenderungen uebernehmen</button>
    </div>
    <div class="card" id="risksCard" hidden>
    ...
    <div class="card" id="testCard" hidden>
      ...
      <button id="runTest" class="mt-10">Pruefschritt ausfuehren</button>
    </div>
        ...
        if (message.type === "result") {
          ...
          if (message.ok && message.structured) {
            ...
            if (data.explanation) {
              ...
              setVisible(explanationCard, true);
            } else {
              setVisible(explanationCard, false);
            }
            ...
            if (data.risks && data.risks.length > 0) {
              ...
              setVisible(risksCard, true);
            } else {
              setVisible(risksCard, false);
            }

            if (data.test_step || data.test_result) {
              setVisible(testCard, true);
...
async function handleAssistantMessage(targetWebview, message) {
  ...
  const allowProjectWorkspaceAnalysis = allowProjectAnalysisWithoutActivePython(payload);
  if (allowProjectWorkspaceAnalysis) {
    payload.v4_context_error = null;
    payload.context_source = "workspace_project_analysis";
    if (!payload.current_file_language) {
      payload.current_file_language = "workspace";
    }
  }
  const contextText = [
    `Datei: ${payload.current_file_path || "keine aktive Datei"}`,
  ...
  if (shouldBlockForMissingPythonContext(payload)) {
    targetWebview.postMessage({
      type: "result",
      ok: false,
      context: contextText,
...
const PROJECT_ANALYSIS_HINT_PATTERNS = [
  /\b(projekt|workspace|workbench|codebase|repository|architektur|struktur|ueberblick|analyse|review|risiko|verbesserung|module|ordner|dateien)\b/i,
  /\b(analysiere|analysieren|bewerte|ueberpruefe|pruefe|untersuche|begutachte)\b/i,
];

const PROJECT_CHANGE_HINT_PATTERNS = [
  /\b(aender|aendere|aendern|fix|behebe|implement|refactor|rewrite|schreib|write|apply|patch|edit|ersetz|insert|delete|loesch|anpass|umsetzen|erstelle|create)\b/i,
];

function isLikelyProjectWorkspaceAnalysisPrompt(promptText) {
  const prompt = String(promptText || "").trim();
  if (!prompt) {
    return false;
  }
  const hasAnalysisHint = PROJECT_ANALYSIS_HINT_PATTERNS.some((pattern) => pattern.test(prompt));
  const hasChangeHint = PROJECT_CHANGE_HINT_PATTERNS.some((pattern) => pattern.test(prompt));
  return hasAnalysisHint && !hasChangeHint;
}

function isMissingPythonContextError(errorText) {
  const text = String(errorText || "");
  return text.includes("Keine aktive Python-Datei") || text.includes("Aktive Datei ist keine Python-Datei");
}

function allowProjectAnalysisWithoutActivePython(payload) {
  if (!payload || payload.mode !== "agent_project") {
    return false;
  }
  if (!payload.v4_context_error || !isMissingPythonContextError(payload.v4_context_error)) {
    return false;
  }
  if (!payload.workspace_root) {
    return false;
  }
  return isLikelyProjectWorkspaceAnalysisPrompt(payload.prompt);
}

function shouldBlockForMissingPythonContext(payload) {
  if (!payload || !payload.v4_context_error) {
    return false;
  }
  if (allowProjectAnalysisWithoutActivePython(payload)) {
    return false;
  }
  return payload.mode === "agent_v4"
    || payload.mode === "agent_v5_lab"
    || payload.mode === "agent_v6"
    || payload.mode === "agent_project";
}
